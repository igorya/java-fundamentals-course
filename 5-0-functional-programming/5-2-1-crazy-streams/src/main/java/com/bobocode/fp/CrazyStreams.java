...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.groupingBy(account -> account.getSex() == Sex.MALE));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .parallel()
                .map(account -> (account.getFirstName() + account.getLastName()).length())
    ...
    public BigDecimal calculateTotalBalance() {
        return accounts.stream()
                ...
                .reduce(BigDecimal.ZERO, (bigDecimal, bigDecimal2) -> bigDecimal2.add(bigDecimal))
                ;
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream()
                .sorted(Comparator.comparing(account -> account.getFirstName() + account.getLastName()))
                .collect(Collectors.toList())
                ;
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                .flatMap(o -> Stream.of(o.getFirstName().split("")))
                .collect(Collectors.groupingBy(
                        s -> s.charAt(0),
                        Collectors.counting()
                ))
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        Predicate<String> predicate = s -> s.length() >= nameLengthBound;

        return Stream.concat(
                        accounts.stream().map(Account::getFirstName).filter(predicate),
                        accounts.stream().map(Account::getLastName).filter(predicate)
                )
                .flatMap(s -> Stream.of(
                        s.toLowerCase().split("")
                ))
